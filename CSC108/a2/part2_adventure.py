...
def generate_map(map_data, p_x, p_y, player_id):
    ...
    map_visual = []
    map_coors = {}
    for i in range(len(map_data)):#outer list
        map_visual.append([])
        for i2 in range(len(map_data[i])):#inner list
            if map_data[i][i2] == '-2':
                map_visual[i].append('(x)')
            else:
                map_visual[i].append('(_)')

    map_visual[p_y][p_x] = '(' +player_id +')'#player_id missing the bracket
            
            
    for index in range(len(map_data)):
        for ind in range(len(map_data[0])):
            key = (ind,index)#creating new pairs
            map_coors[key] = float(map_data[index][ind])#!!!!convert it into a float!!!!
...
def load_locations(filename):
    ...
    new_key = float(line) #new_key = 0.0
    ...
    while line:
        ...
        if line == '[BEGIN DESCRIPTION]':
            ...
            line = f.readline().strip()#either new ID or begin action
            if line == '[BEGIN ACTIONS]':#begin action
                ...
                while line != '[END ACTIONS]':#use loop to get every line of actions
                    lst = line.strip().split(',')
                    lst[1] = float(lst[1])#convert action ID into a float!!!!!!!!!!!
...
def update_map(map_visual, map_coors, player_id, px, py, dx, dy):
    ...
    if px + dx >= len(map_visual[0]) or px +dx < 0 or \
       py + dy >= len(map_visual) or py + dy < 0:#invalid input
        return (px,py)
...
def check_items(current_location, game_items, inventory):
    ...
    index = None #have to assign it first!!
    for i in range(len(values)):
        if float(current_location) == float(values[i][1]) or \
           float(current_location) == float(values[i][2]):#current_location was an int!!
           index = i #to record the index
    if index != None:
        if keys[index] not in inventory:#items in inventory should not repeat!!!
...
def check_game_won(game_data, inventory, p_x, p_y):
    ...
    '''
    for item in list(game_data["game_items"].keys()):#game_data["game_items"] was an dict!!!
    ...
    for key in game_data["location_data"]:
        if current_location == key:
            ...
            if decision > len(values[1]):
                ...
                new_location_id = values[1][decision-1][1]#decision-1
