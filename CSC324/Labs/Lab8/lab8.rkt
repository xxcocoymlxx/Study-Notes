...
(cond
    [(>= n 5) (-< (cons 5 (make-change-helper (- n 5)));option 1
                  (make-list n 1))];option 2
    [else
     (make-list n 1)]))
...
(define (make-change-gen-helper coins n)
  (cond
    ...
    [(equal? 1 (length coins))
     (if (equal? (modulo n (first coins)) 0)
                       (make-list (/ n (first coins)) (first coins))
                       (fail))]
    ...
    [else
     (if (> n (first coins))
         (cons (first coins) (make-change-gen-helper coins (- n (first coins))))
         (let*
             ([choice (-< (list coins (- n (first coins))) (list (rest coins) n))])
           (apply make-change-gen-helper choice)))]))
