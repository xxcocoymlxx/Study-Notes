...
(define (n-copies s n)
  (cond 
    [(< n 1) ""]
    [(= n 1) s]
    [else (string-append s "" (n-copies s (sub1 n)))]))
...
(define (num-evens numbers)
  (cond
    [(null? numbers) 0]; In this case the list is empty. What should be returned?
    [else
  ...
  (cond
    ...
    [else
     (let ([first-list (first lists-of-numbers)]
           [rest-num-many-evens (num-many-evens (rest lists-of-numbers))])
       (if (> (num-evens first-list) 2)
           (+ rest-num-many-evens 1)
           rest-num-many-evens))]))
...
(define (double-list numbers)
  (if (null? numbers)
      ...
      ; Hint: construct a list out of the first part of
...
(define (calculate expr)
  (if (number? expr)
      (+ 0 expr);if the expression is a single number
       (cond
          [(equal? '+ (first expr)) (+ (calculate (second expr)) (calculate (third expr)))]
          [(equal? '- (first expr)) (- (calculate (second expr)) (calculate (third expr)))]
          [(equal? '* (first expr)) (* (calculate (second expr)) (calculate (third expr)))]
          [(equal? '/ (first expr)) (/ (calculate (second expr)) (calculate (third expr)))]
      );the "else" case does NOT need another perenthesis!!! a (cond ...) is enough!
   )
 )

(module+ test
  (test-equal? "calculate: a single number"
               (calculate 6)
               6)
