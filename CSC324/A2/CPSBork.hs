...
facEnv :: Env
facEnv = def [("fac", Lambda ["n"] (If (Equal (Var "n") (Literal $ Num 0)) (Literal $ Num 1) (Times (Var "n") (App (Var "fac") [(Plus (Var "n") (Literal $ Num (-1)))]))))]
...
-- function definition:
cpsExpr (Lambda params body) s context = addk(cpsExpr body "lambda_" (\cps_body -> context (Lambda ((map rename params) ++ ["k"]) cps_body)))
...
-- if expressions
cpsExpr (If cond conseq altern) s context = cpsExpr cond s (\cps_cond -> (context (If cps_cond (cpsExpr conseq s (\result1 -> (context result1))) (cpsExpr altern s (\result2 -> (context result2))))))

-- | Helper function that renames a variable by prepending "cps_"
rename :: String -> String
rename s = "cps_" ++ s

-- | Helper function that add (App (Var "k")) for a function

addk (Literal v) = id(App (Var "k") [Literal v])
addk (Plus a b) = id(App (Var "k") [Plus a b])
addk (Times a b) = id(App (Var "k") [Times a b])
addk (Equal a b) = id(App (Var "k") [Equal a b])
addk (Var v) = id(App (Var "k") [Var v])
addk (If cond conseq alter) = id(If cond (addk conseq) (addk alter))
addk (Lambda argums body) = id(Lambda argums (addk body))
addk (App v (x:xs)) = id(App v (init(x:xs) ++ [addk (last xs)]))
